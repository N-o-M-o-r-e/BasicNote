package com.project.tathanhson.basicnote.legacy
...
object Migrations {
    ...
    fun getPreviousNotes(app: Application): List<BaseNote> {
        ...
        getNotePath(app).listFiles()
            ?.mapTo(list) { file -> XMLUtils.readBaseNoteFromFile(file, Folder.NOTES) }
        getDeletedPath(app).listFiles()
            ?.mapTo(list) { file -> XMLUtils.readBaseNoteFromFile(file, Folder.DELETED) }
        getArchivedPath(app).listFiles()
            ?.mapTo(list) { file -> XMLUtils.readBaseNoteFromFile(file, Folder.ARCHIVED) }
        return list
    }

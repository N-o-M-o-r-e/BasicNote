...
import com.project.tathanhson.basicnote.activities.MakeList
import com.project.tathanhson.basicnote.activities.TakeNoteView
import com.project.tathanhson.basicnote.databinding.DialogColorBinding
...
import com.project.tathanhson.basicnote.recyclerview.adapters.ColorAdapter
...
import java.io.File
import com.project.tathanhson.basicnote.preferences.View as ViewPref


abstract class NoteViewFragment : Fragment(), ItemListener {
    ...
    override fun onClick(position: Int) {
        if (position != -1) {
            adapter?.currentList?.get(position)?.let { item ->
                if (item is BaseNote) {
                    when (item.type) {
                        Type.NOTE -> goToActivity(TakeNoteView::class.java, item)
    ...
    internal fun showFileOptionsDialog(file: File, mimeType: String) {
